...
//Voltage offset of A‚ lCs analog comparator (in mV): -50 up to 50
#define COMPARATOR_OFFSET     15
//Capacitance of the probe tracks of the PCB and the A‚ lC (in pF)
#define CAP_PCB               42
...
const unsigned char Splash_str[] PROGMEM = "Components";
const unsigned char Version_str[] PROGMEM = "Tester";
  ...
  /*
     ...
     Solution:
      ...
      - For a MOSFET pretection diode we have to make sure that the MOSFET
        ...
        channel FETs.
      - Take care about the internal voltage drop of the A‚ lC at the cathode
      ...
      - Filter out caps by checking the voltage before and after measurement
        with Rh. In 15ms a 22A‚ lF cap would be charged from 0 to 7mV, a larger
      ...
      - Rl drives a current of about 7mA. That's not the best current for
        measuring Vf. The current for Rh is about 10.6A‚ lA.
        Most DMMs use 1mA.
  ...
  */
  ...
  while (Mode > 0)
  {
    ...
    while (Counter < 100)
    {
      ...
      ADC_DDR = Probes.ADC_2;                    //Pull-down probe-2 directly
      R_PORT = Probes.Rl_1;
      //Start ADC conversion, ADC performs S&H after 1.5 ADC cycles (12A‚ ls)
      ADCSRA |= (1 << ADSC);                     //Start conversion
      //Wait 20A‚ ls to allow the ADC to do it's job
      waitus(20);
      //Stop pulse
      R_PORT = 0;
      ADC_DDR = Probes.ADC_2 | Probes.ADC_1;
      //Get ADC reading (about 100A‚ ls)
      while (ADCSRA & (1 << ADSC));              //Wait until conversion is done 
      Value += ADCW;                             //Add ADC reading 
      //Wait
      waitus(900);
...
void CheckResistor(void)
{
  ...
  /*
     Resistor measurement
      ...
      - For low resistance consider the internal resistors of the A‚ lC
        for pulling up/down.
...
//Measure cap >4.7A‚ lF between two probe pins
byte LargeCap(Capacitor_Type *Cap)
{
...
byte SmallCap(Capacitor_Type *Cap)
{
  ...
  /*
     ...
     use the A‚ lCs internal 16-bit counter and analog comparator. The counter
  ...
  */
  ...
  if (Flag == 3)
  {
    ...
    /*
       ...
       bandgap reference if C is 100nF up to 20A‚ lF. The minimum of 100nF
        ...
        ((Scale == -9) && (Value <= 20000)))
    {
      ...
      */
      ...
      /*
         ...
         the voltages of the cap and the bandgap reference. Since the A‚ lC
         has an internal voltage drop for the bandgap reference the
         A‚ lC used actually U_bandgap - U_offset. We get that offset by
...
void MeasureCap(byte Probe1, byte Probe2, byte ID)
{
  ...
  */
  ...
  if (Check.Diodes == 0)
  {
    ...
    if (Check.Found == COMP_RESISTOR)
    {
      //Report capacitor for large C (> 4.3A‚ lF)
...
byte MeasureInductance(uint32_t *Time, byte Mode)
{
  ...
  */
  ...
  if (Mode & MODE_DELAYED_START)                 //Delayed start
  {
    ...
    ADC_PORT = Probes.ADC_1;                     //Pull up probe-1 directly
    /*
       Delay timer by about 3-4A‚ ls to skip capacitive effects of large inductors
        - a single loop needs 4 cycles, the last loop run just 3
        - cycles burnt: <MCU cycles per A‚ ls> * 4 - 1
    */
    while (Test > 0)
    {
      Test--;
  ...
  //Convert counter (MCU cycles) to time (in A‚ ls)
  if (Counter > 0)
  {
  ...
  while (Counter < Check.Diodes)                 //Check all diodes
  {
    ...
    if ((Diode->A == A_Pin) && (Diode->C == C_Pin))
    {
      ...
      /*
         ...
         measurements for a virtual test current. Low test current is 10A‚ lA
